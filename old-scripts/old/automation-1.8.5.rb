...
def extractConfigDetails(automation_config_file)
  ...
  @outputRenamed = "" ##* (For testing) = @path+"output-old" & comment drcomp
  @book_diff_folder=""
  @updateFolder=""
  @book_update_folder=""
...
def createFolders(path)
  ...
  update_ts = update_ts.split("_+").first
...
def copyFolderContents(src,dest)
  ...
  for file in @files
    ...
    # Copy file
    if !file.include?'deletedList.list'
...
def update_limit_check() 
  ...
  diff_size=accepted_size-updates_size
  if(diff_size>0)
