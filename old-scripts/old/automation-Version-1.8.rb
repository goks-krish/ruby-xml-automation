...
def extractConfigDetails(automation_config_file)
  ...
  @outputRenamed = @path+"output-old" ##* (For testing) = @path+"output-old" & comment drcomp
  @book_diff_folder=""
  @book_update_folder=""
  @book_zip_generated_folder=""
  @book_prod_final_zip_folder=""
  @book_prod_final_update_folder=""
end
...
def createZip(zip_src_path,zip_des)
...
#  print "Zip done successfully"  
  if system("java -jar zipUtil.jar #{zip_src_path} #{zip_des}")
    print "Zip success\n"
  else
    print "Zip Failed\n"
    exit;
...
def delete_update_folder(diff_size,prod_folder,final_book_zip)
  deleted_size = 0;
  xmldoc = XmlSimple.xml_in(@config_file)
  update_ts_array=xmldoc['update_timestamp']
  update_ts_array.each do |time_value|
    ts= Time.parse(time_value)
    update_ts=ts.to_s()
    update_ts = update_ts.gsub(":","-");
    update_ts = update_ts.gsub(" ","_");
    file_name=prod_folder+"update_"+update_ts+"/"+@book_full_name+"_update_"+update_ts+".zip"
    if File::exist?(file_name)
      if(deleted_size<diff_size)
        puts "Deleted :"+file_name
        deleted_size=  deleted_size+File.size(file_name)
...
if @new_book_operation==false
  update_limit_check(@book_prod_final_zip_folder+@book_full_name+".zip",@book_zip_generated_folder)
end
